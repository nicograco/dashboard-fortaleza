...
jugador_seleccionado = st.sidebar.selectbox(
    "Seleccionar Jugador", lista_jugadores
)
...
total_min = (
    df_jugador[c_min].sum()
    if c_min and pd.api.types.is_numeric_dtype(df_jugador[c_min])
    else len(df_jugador) * 90
)
total_goles = (
    df_jugador[c_goles].sum()
    if c_goles and pd.api.types.is_numeric_dtype(df_jugador[c_goles])
    else 0
)
total_autogoles = (
    df_jugador[c_autogoles].sum()
    if c_autogoles and pd.api.types.is_numeric_dtype(df_jugador[c_autogoles])
    else 0
)
total_asist = (
    df_jugador[c_asist].sum()
    if c_asist and pd.api.types.is_numeric_dtype(df_jugador[c_asist])
    else 0
)
...
maxvel = (
    df_jugador[buscar_col(df_jugador, ["maxvel"])].max()
    if buscar_col(df_jugador, ["maxvel"])
    else 0.0
)
...
# BUSQUEDA INTELIGENTE DE FOTO CON VALIDACIÓN ESTRICTA
...
if carpeta_fotos_real:
  ...
  palabras_jugador = [
      p.lower() for p in str(jugador_seleccionado).split() if len(p) > 2
  ]
  ...
  if archivo_encontrado:
    ...
    mensaje_estado = (
        f"¡Foto cargada para {jugador_seleccionado} ({archivo_encontrado})!"
    )
  else:
    mensaje_estado = f"Sin fotografía registrada para {jugador_seleccionado}."
else:
  mensaje_estado = "No se encontró la carpeta FOTOS."

# Si no hay foto real, asignamos una imagen por defecto genérica (silueta o placeholder) y evitamos robar la foto de otro
...
with col_foto:
  ...
  st.markdown(
      f"<h4 style='text-align: center; margin-top: 8px; color:"
      f" #111;'>{jugador_seleccionado}</h4>",
      unsafe_allow_html=True,
  )
